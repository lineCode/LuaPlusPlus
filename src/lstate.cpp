...
{
  lua_MainThread* mainThread = LuaAllocator<lua_MainThread>::alloc(nullptr, LUA_TTHREAD, sizeof(lua_MainThread));
...
{
  L = L->globalState->mainthread;  /* only the main thread can be closed */
  lua_lock(L);
  close_state(L);
}
